...
- Phase 3 mobile layering uses the USER-created/verified `home-mobile-layered-base-v1.png` and `home-mobile-layered-overlay-v1.png` with its own responsive portrait 28-instance wave configuration while reusing the eight shared wave deliveries. USER mobile QA accepts speed and concentration; the previous warm tint direction reduced visibility against yellow, so the current correction uses white, sky-blue, and sky-cyan mobile-only variants for large/background families 01, 02, and 08, with final visual re-review pending.
