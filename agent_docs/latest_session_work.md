...
The current branch implements the approved Home background frames B/C with the canonical `A → B → C → A` CSS-driven stepped loop. The initial candidate is a 3.6-second cycle with approximately 1.2 seconds per state; reduced motion disables the loop and freezes Frame A. The approved PNG bytes, Home composition, Schrödinger interaction, and existing six deterministic reduced-motion QA captures remain unchanged in intent.
